...
plot_numeric_numeric <- function(data, #  data frame or tibble (obligatory parameter)
                                 ...
                                 hex_bins = 10, # number of horizontal and vertical bins in hexagonal heatmap
        ...
        # ------------------------------------------------------------------------------           
          ...
          if (save_plots == TRUE){
            if (base::is.null(save_filename)){
              ggplot2::ggsave(filename = base::paste0(base::paste(base::Sys.Date(), numeric_var_1_name, numeric_var_2_name), save_file_format),
                              plot = plots, width = save_width, height = save_height, units = save_plots_units, dpi = save_dpi)
              plot_name <- base::paste0(base::paste(base::Sys.Date(), numeric_var_1_name, numeric_var_2_name), save_file_format)
